...
public class RAPGA : IMetaheuristic
{
    ...
    public async Task<ISolution> SolveAsync(IProblem problem, CancellationToken ct = default)
    {
        ...
        _fitnessEvaluator = baseFitnessEval;
        ...
        while (iteration < _config.MaxIterations && !token.IsCancellationRequested)
        {
            ...
            while (effort < maxEffort && nextPop.Count < _config.MaximumPopulationSize)
            {
                effort++;
                ...
                bool betterThanParents = IsBetter(child, parentA.ToSolution(), parentB.ToSolution(),
                    _config.ComparisonFactor, baseFitnessEval.Minimize);
            ...
            if (IsBetter(globalBest, currentBest, baseFitnessEval.Minimize))
            ...
            if (iteration % Math.Max(1, _config.ProgressInterval) == 0 || iteration == _config.MaxIterations || iteration == 1)
            {
                ...
                var fitnessValues = nextPop.Select(s => s.Fitness!.Value).ToList();
                var meanFitness = fitnessValues.Sum() / fitnessValues.Count;
                var medianFitness = fitnessValues.OrderBy(f => f).ElementAt(fitnessValues.Count / 2);
                var worstFitness = fitnessValues.Min();
    ...
    private static bool IsBetter(ISolution a, ISolution b, bool minimize)
    {
        if (a.Fitness is null || b.Fitness is null) return false;
        var bIsBetter = a.Fitness.Value < b.Fitness.Value;
        if (minimize) return bIsBetter;
        return !bIsBetter;
    }

    private static bool IsBetter(ISolution child, ISolution p1, ISolution p2, double comparisonFactor, bool minimize)
    {
        if (child.Fitness is null || p1.Fitness is null || p2.Fitness is null)
        {
            return false;
        }

        if (comparisonFactor is < 0.0 or > 1.0)
            throw new ArgumentOutOfRangeException(nameof(comparisonFactor));

        double worstParentFitness = Math.Min(p1.Fitness.Value, p2.Fitness.Value);
        double bestParentFitness = Math.Max(p1.Fitness.Value, p2.Fitness.Value);
        double threshold = worstParentFitness + comparisonFactor * (bestParentFitness - worstParentFitness);

        return child.Fitness.Value > threshold;
    }
