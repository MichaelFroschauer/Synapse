...
// Notes:
// - This adapter expects GeneticSharp to be referenced in your project (NuGet package: GeneticSharp)
// - You must provide a prototype chromosome factory (that can create random chromosomes) and a mapping
//   function that converts an IChromosome to ISolution.
// - GeneticSharp maximizes fitness; our framework assumes lower fitness is better (e.g. distances).
//   The adapter therefore maps raw fitness -> maximizing fitness using a safe transform: 1/(1+raw).
    ...
    public Task<ISolution> SolveAsync(IProblem problem, CancellationToken ct = default)
    {
        ...
        generationHandler += (_, _) =>
        {
            try
            {
                ...
                // pause: busy-wait until resumed (GeneticSharp has Pause/Resume but to keep this adapter lightweight
                // we simply spin-sleep here). If you want true pause semantics call _gsGeneticAlgorithm.Pause()/_gsGeneticAlgorithm.Resume().
