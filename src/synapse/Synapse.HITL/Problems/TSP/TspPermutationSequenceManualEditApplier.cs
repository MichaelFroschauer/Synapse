...
public class TspPermutationSequenceManualEditApplier : IManualEditApplier
{
    public ISolution Apply(ISolution candidate, ISolution manual)
    {
        ...
        // 1) Filter manual sequence: only valid cities, no duplicates (keep first occurrence)
        ...
        // 2) Remaining cities in the order of baseTour (excluding manual cities)
        var remaining = baseTour.Where(c => !manualSeq.Contains(c)).ToList();

        // 3) Determine insertion position in the original baseTour: first occurrence of any manual city
        ...
        // 4) Calculate insertion position within 'remaining'
        ...
        // 5) Build result: remaining[0..remInsertIdx-1] + manualSeq + remaining[remInsertIdx..end]
        ...
        // 6) Rotate so that the original starting city of the candidate tour remains at index 0
