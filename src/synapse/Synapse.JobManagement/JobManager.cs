...
public class JobManager : IJobManager, IDisposable
{
    ...
    private class JobEntry
    {
        ...
        /// <summary>Guard to prevent multiple concurrent PauseJobAsync calls from interaction triggers.</summary>
    ...
    private async Task RunJobAsync(JobEntry entry)
    {
        ...
        try
        {
            ...
            algorithm.ProgressChanged += (s, progressArgs) =>
            {
                ...
                if (progressArgs.Diversity.HasValue)
                {
                    info.DiversityPerIteration.Add(new IterationDiversity(progressArgs.Iteration, progressArgs.Diversity.Value));
                }
    ...
    private async Task SaveSnapshotIfConfigured(Guid jobId, JobInfo info)
    {
        ...
        try
        {
            await _snapshotStore.SaveSnapshotAsync(jobId, info);
        }
        catch (Exception ex)
        {
